...
Будет открыта страница сравнения версий. Левая колонка отображает начальную версию файла, которая сейчас находится в основном репозитории. Правая колонка отображает изменения внесенные в новый файл. Плюсами отмечены добавленные строки. Если изменения нас устраивают, то нажимаем кнопку ``New Issue``. После этого будет выведена информации о завершении слияния.
...
Не путайте автоматическую публикацию с автоматической сборкой (генерацией) самой документации Sphinx, об этом подробнее смотрите раздел :ref:`sphinx-autobuild`.
